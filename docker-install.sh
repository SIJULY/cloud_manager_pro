...
update_panel() {
    ...
    # ==============================================================
    # ✨ 核心修改 1：同步拉取最新版的安装脚本自身，覆盖当前运行的文件
    # ==============================================================
    ...
    # ==============================================================
    # ✨ 核心修改 2：强制将本地仓库的远程地址指向最新的 Pro 版仓库
    # ==============================================================
    print_info "正在同步并强制修复远程仓库地址..."
    ...
    docker compose down --remove-orphans
    if docker compose up -d --build; then
        print_success "更新完成！系统已恢复运行，老仓库已自动迁移，您的数据已全部保留。"
    else
        print_error "更新失败。"
    fi
}
...
echo "  3) 更新: 升级面板容器与代码 (自动迁移老仓库并更新本脚本)"
echo "  4) 卸载: 删除所有文件和容器"
echo "  5) 退出"
echo "=========================================================="
